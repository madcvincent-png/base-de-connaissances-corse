...
function clearHover(){
  if (hoveredCommuneLayer){
    ...
    try{ hoveredCommuneLayer.closeTooltip && hoveredCommuneLayer.closeTooltip(); }catch(e){}
...
function renderCommunes(){
  ...
  communesLayer = L.geoJSON(state.communesGeo, {
    ...
    onEachFeature: (feat, layer) => {
      ...
      layer.on("mouseover", () => { try{ layer.openTooltip && layer.openTooltip(); }catch(e){}; hoverCommune(layer); });
      layer.on("mouseout", () => { try{ layer.closeTooltip && layer.closeTooltip(); }catch(e){}; clearHover(); });
