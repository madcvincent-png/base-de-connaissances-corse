...
function updateLabels(){
  ...
  const major = new Set(["ajaccio","bastia","porto-vecchio","calvi","corte"]);
  if (z <= 8){
    const majors = state.communesMeta
      .filter(c => major.has(normalize(c.nom)))
  ...
  const maxLabels = (z <= 9) ? 30 : (z <= 11 ? 80 : 160);
    ...
    <div class="comm-details" ${expanded ? "" : "hidden"}>
      <div class="details">
        ...
        <div style="margin-top:10px">
          <a href="${ficheUrl}" target="_blank" rel="noopener">Ouvrir la fiche commune</a>
        </div>
      </div>
    </div>
  </div>`;
